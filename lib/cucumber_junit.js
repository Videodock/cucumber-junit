 ...
 */
function createFailure (message, screenshotPath) {
 ...
 * @param  {Object}    featureJson  Feature output from Cucumber.JS
 * @param  {Object}    options      if `strict` is true, pending or undefined steps will be reported as failures
 * @param  {string}    [screenshot] Screenshot data found in step embeddings
 * @return {Array}                  Array of elements for an XML element <testcase>
 */
function convertStep (stepJson, scenarioJson, featureJson, options, screenshot) {
    var stepOutput = [{
            _attr: {
                name:      stepJson.keyword + stepJson.name,
                classname: options.prefix + featureJson.name + ' - ' + scenarioJson.name
            }
        }],
    ...
    if (screenshot) {
        var cwd = process.cwd(),
            screenshotDir = path.join(cwd, options.directory),
            stream;

        screenshotPath = path.join(screenshotDir, scenarioJson.id + '.jpg');

        if (!fs.existsSync(screenshotDir)) {
            fs.mkdirSync(screenshotDir);
        }

        stream = fs.createWriteStream(screenshotPath);
    ...
    case 'passed':
        break;
    case 'failed':
        stepOutput.push(createFailure(result.error_message, screenshotPath));
        break;
    case 'pending':
    case 'undefined':
        if (options.strict) {
 ...
 */
function convertScenario (featureJson, scenarioJson, options) {
    var scenarioOutput = [{
        _attr: {
            name:     options.prefix + featureJson.name + ' - ' + scenarioJson.name,
            tests:    0,
            failures: 0,
            skipped:  0
        }
    ...
    if (scenarioJson.steps) {
        ...
        var screenshotStep = scenarioJson.steps
            .find(function (stepJson) {
                return stepJson.embeddings && stepJson.embeddings.length;
            }), screenshot;

        if (screenshotStep) {
            screenshot = screenshotStep.embeddings[0].data;
        }
        ...
        scenarioJson.steps
            ...
            .forEach(function (stepJson) {
                ...
                var testcase = convertStep(stepJson, scenarioJson, featureJson, options, screenshot);
...
function convertFeature (featureJson, options) {
    ...
    return elements
        ...
        .map(function (scenarioJson) {
            var scenario = convertScenario(featureJson, scenarioJson, options);
            if (featureJson.uri) {
